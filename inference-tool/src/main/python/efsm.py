...
def efsm(nfsm : nx.Graph) -> EFSM:
    ...
    for (origin,dest,ip_sig,op_sig), group_samples in observations.groupby(["origin", "dest", "ip_sig", "op_sig"]):
        ...
        efsm[origin][(ip_sig,op_sig,dest)] = Transition(ip_arity,op_arity,pd.concat([registers,ip_args,op_args],axis=1),update)
...
def __initialisation(nfsm : nx.Graph) -> pd.DataFrame :
    ...
    initial = pd.DataFrame([{"state" : state, "configuration" : __unquote(nfsm.nodes[state]["configuration"])} for state in initial])
...
def __samples(nfsm : nx.Graph) -> pd.DataFrame :
    ...
    updates     = expand_configuration(transitions,"update").apply(lambda update : [exp if exp is not None else f"r{index}" for index,exp in enumerate(update)])
