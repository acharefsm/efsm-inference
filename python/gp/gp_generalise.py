...
def infer_output(samples, counter=None, **kwargs):
    ...
    print('inferred output function: ', str(best))
    ...
    if not correct:
        ...
        if counter:
            counter.increment()
        print("best inferred output function ",str(best))
...
def __formula_to_tree(exp, pset, rename={}):
    ...
    """
    try:
        exp = deap_gp.PrimitiveTree.from_string(exp, pset)
    except:
        print(exp)
        exp = deap_gp.PrimitiveTree.from_string(infix_to_prefix(exp), pset)
    rename = {k: deap_gp.Terminal(v, None, object) for k, v in rename.items()}
    for inx, element in enumerate(exp):
        if isinstance(element, deap_gp.Terminal) and element.format() in rename:
            exp[inx] = rename[element.format()]
    assert "r_b" not in str(exp), f"{exp}: {rename}"
    return deap_gp.graph(exp)
